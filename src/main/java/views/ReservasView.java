...
public class ReservasView extends JFrame {
	...
	public ReservasView() {
		...
		lblCheckIn.setBounds(68, 136, 197, 14);
		lblCheckIn.setFont(new Font("Roboto Black", Font.PLAIN, 18));
		panel.add(lblCheckIn);
	
		JLabel lblCheckOut = new JLabel("FECHA DE SALIDA");
		lblCheckOut.setForeground(SystemColor.textInactiveText);
		lblCheckOut.setBounds(68, 221, 197, 14);
		...
		txtFechaSalida.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
		        ...
		        long diferenciaMillis = txtFechaSalida.getDate().getTime() - txtFechaEntrada.getDate().getTime();
		        double horas = (double) diferenciaMillis / (1000 * 60 * 60);
		        double valorTotal = horas * tarifaPorHora;
		...
		txtValor.setBounds(78, 328, 143, 33);
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(Double.valueOf(txtValor.getText())<0) {
					JOptionPane.showMessageDialog(null, "No puedes elegir una fecha anterior a la ingresada en la entrada");
					
				}else if(ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {	
					   btnsiguiente.setVisible(false);
					RegistroHuesped registro = new RegistroHuesped();
			
					reservas reserva = new reservas(txtFechaEntrada.getDate(),txtFechaSalida.getDate(),Double.valueOf(txtValor.getText()),(String) txtFormaPago.getSelectedItem());
					 EntityManager em = JPAUtils.getEntityManager();
			            em.getTransaction().begin();
			            em.persist(reserva);
			            em.getTransaction().commit();
			            
			            // Obtener el ID de la reserva después de guardarla
			            long reservaId = reserva.getId(); // Asegúrate de que la entidad "reservas" tenga un método getId()
			            System.out.println("ID de la reserva: " + reservaId);
			            registro.setReservaYId(reserva, reservaId);
